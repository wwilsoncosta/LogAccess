...
{
    public class LogAccessService : ILogAccessRepository
    {
        private static string? _conn = "";
        private readonly IConfiguration _configuration;

        public LogAccessService(IConfiguration configuration)
        {
            _configuration = configuration;
            _conn = _configuration.GetSection("ConnectionStrings").GetSection("DefaultConnection").Value;
        }
        ...
        public List<LogAccessCount> GetAccessHour(List<LogAccess> logAccesses)
        {
            ...
            for (int i = 0; i < 23; i++)
            {
                ...
                var itemSelected = logAccesses.FirstOrDefault(x => x.DataHoraAcesso.Hour == i);
                if (itemSelected != null)
                {
                    LogAccessCount logAccessCount = new LogAccessCount(count.Count, HourFormat(itemSelected.DataHoraAcesso.Hour));
                    listLogAcess.Add(logAccessCount);
                }
                else
                {
                    LogAccessCount logAccessCount = new LogAccessCount(0, HourFormat(i));
                    listLogAcess.Add(logAccessCount);
                }
        ...
        public string HourFormat(int hora)
        {
            if (hora <= 9)
                return "0" + hora + ":00";
            else
